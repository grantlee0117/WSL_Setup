...
> 3. `core.autocrlf=input`：提交时把 Windows 的 CRLF 换行符转换成 LF、检出时不反向转换。Windows 上更常见的默认值是 `true`（检出再转回 CRLF）；但本仓库的开发以 WSL / Linux 为主，仓库内统一用 LF 最省事，所以选 `input` 而非 `true`，避免在 WSL 里又混进 CRLF。
