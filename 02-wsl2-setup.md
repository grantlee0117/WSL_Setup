...
> 1. 启用 WSL 功能和虚拟机平台
> 2. 安装 WSL2 内核
> 3. 默认安装 Ubuntu 发行版（一个完整的 Linux 操作系统）
>
> 也就是说，`wsl --install` 一条命令搞定了所有事情，不需要额外手动安装 Linux 发行版。

安装完成后**重启电脑**。重启后会自动弹出 Ubuntu 窗口，要求你设置 Linux 用户名和密码。
...
| 全选删除 | `Ctrl+6` 设起点，`Alt+}` 到文末，`Ctrl+K` 删除 | 如果文件有旧内容需要清空 |
