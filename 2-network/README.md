...
> **本域作用**：集中放置代理与 DNS 的**全部配置与排错**。无论 Windows 侧还是 WSL 侧的网络问题，都来这里查——单一信源，少跑冤枉路。
>
> **什么时候做哪部分**（数字前缀代表域顺序，不是严格时间顺序）：
> - **Windows 侧代理**：软件的*安装*在 [1-windows](../1-windows/README.md) §5.3；安装之后的*订阅与配置*见下方「一、Windows 侧代理」。
> - **WSL 侧代理与 DNS**：在 [3-wsl](../3-wsl/README.md) 装好 WSL 并配好 §2.1 `wsl.conf` **之后**进行，见下方「二、WSL 侧代理与 DNS」。

---

## 一、Windows 侧代理（Clash Verge 等）

代理软件本身的安装见 [1-windows §5.3](../1-windows/README.md)。订阅、节点、GUI 配置、TUN 模式、链式代理等，参考本目录 `梯子配置/` 下的现成文档与配置：

- `梯子配置/LapTop/文档/` — Clash Verge GUI 配置图文（订阅、模式、设置等）
- `梯子配置/LapTop/文档/静态住宅IP链式代理完整配置指南.md` — 链式代理配置
- `梯子配置/LapTop/配置/` — 现成的 `verge.yaml` / `Merge.yaml` / `Script.js` 等配置文件
- `梯子配置/IOS/文档/iPhone配置.md` — iOS 侧配置

> Windows 侧 Clash 开启 **TUN 模式**后，会在网络层兜底拦截所有流量（包括 WSL 经镜像模式共享出来的流量），相当于代理的总开关。
...
> **如果验证不通过**：先在 PowerShell 中 `wsl --shutdown` 重启（让 boot command 重新执行），再检查。详见下方「三、网络健康检查」和「四、网络故障排查 FAQ」。
...
先区分是 DNS 问题还是连通性问题。如果报 `Temporary failure in name resolution`，是 DNS 坏了（见上方「二、WSL 侧代理与 DNS · DNS 原理说明」）。如果 DNS 正常但连接超时，说明直连 github.com:22 被阻断，需要在 `~/.ssh/config` 里配 ProxyCommand（见上方「二、WSL 侧代理与 DNS · ③ Git SSH 代理」）。`http_proxy` 只对 HTTPS 协议生效，SSH 不读这个变量。
