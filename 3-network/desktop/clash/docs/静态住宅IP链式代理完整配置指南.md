...
在正式启用之前，我们需要了解 Clash 的三种主要代理模式，并选择最适合日常使用的一种：
...
很多人刚接触代理时会混淆"节点"和"服务器"这两个词，觉得它们好像是一回事。实际上它们处于不同的抽象层次，理解这个区别对后续配置链式代理非常重要。

服务器是一个物理或虚拟的计算实体，它真实存在于某个数据中心或机房里，有自己的 CPU、内存、硬盘和网络接口。当我们说"一台位于东京的服务器"，指的是一台实实在在运行着的机器，它有固定的 IP 地址，上面可能跑着各种服务程序。服务器是基础设施层面的概念，是承载一切网络服务的硬件载体。
...
这就是本项目要实现的核心功能。接下来我们将购买静态住宅 IP，并配置链式代理。
...
Merge 是 Clash Verge 的特有功能，它在**订阅加载后、规则生效前**，把你的自定义内容"合并"进去。
...
### 7.5 不要在 prepend-rules 里放 MATCH
...
## 第八章：配置 dialer-proxy 链式代理

### 8.1 dialer-proxy 是什么？
