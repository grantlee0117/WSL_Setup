...
2. 或者可以在左侧菜单 → **代理**，将策略组下拉菜单点开之后，再选择具体的节点进行连接测试和使用。
3. "首页"菜单左侧中间"网络设置"板块 → 点击 **系统代理** → 开启；
4. "首页"菜单左侧中间"网络设置"板块 → 点击 **虚拟网卡模式** → 开启，这样可以劫持所有流量，包括 API 调用和命令行；该功能视个人需要而选择是否开启，开启之后，即使不走系统代理的流量——如终端或其他不遵循系统代理设置的软件——也会被强制接管转发。
...
## 第七章：Script.js 要改的三处代码

> 本章只讲 `Script.js` 里需要你改的三处代码；**完整部署步骤（导入 `Merge.yaml` / `Script.js` 并启用）见第十章**。先把这三处的逻辑弄清楚，到第十章照着填即可。

填入静态住宅IP信息时要改三处，且互相关联：

第一处，将 type、server、port、username、password 字段替换为你购买的静态住宅IP节点信息（不要幻想白嫖，下面这个数字是瞎填的）。然后你要考虑一个问题：是否把这个静态住宅IP节点的名字改掉。这里自定义的名字为"原生静态住宅IP-美国洛杉矶"，你也可以改成别的，但一旦改了名字，后续还有两处代码要跟着改，同时还得注意不能和机场节点名字冲突，否则会报错。
...
改完这三处后，按**第十章**的步骤把 `Merge.yaml` / `Script.js` 导入并启用即可。
...
在部署之前，按**第七章**的说明把你的静态住宅IP信息填入 `Script.js`：

1.  用文本编辑器（如 Notepad、VS Code）打开 `Script.js`。
2.  找到 `const customProxies = [...]` 部分，按第七章「第一处」填入 `server` / `port` / `username` / `password`。
3.  `dialer-proxy` 保持默认 `"美日自动-链式代理一级跳"`（指定流量必须先经过机场节点）。新手若不改 `name` 字段，第七章的第二、三处就无需改动。
